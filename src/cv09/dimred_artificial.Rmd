...
summary(pca_res)
# first two new features captures 98.53 % of variance

# Question: the PCA result has the field called "x". What do you think it represents?
# It represents the transformed features (onto new axes)
# pca_res$x
...
# PCA, in fact, tranforms the features into a different space, in which each axes captures some part of the data varaince (information). The axes are ordered by the proportion of varaince they capture. Axis PC1 the biggest part, axis PCn the smallest part.
# Additionally, the new axes (no each pair of them) are NOT correlated.
# Each brings new unique information (the amount , they are indendent.
...
# unrelated note: corelation in regression > betas have a big range where the model still have the same performance
```

```{r}
# An important output of PCA is the transformation matrix, accessible in the field "rotation"
pca_res$rotation
# PCA's transformation matrix is rotation
# rotation it is a linear transformation
# we can tell that the new axes are a linear combination of the old axes

# rottion matrix column vector norms
sqrt(colSums(pca_res$rotation^2))
...
transform_mat2 <- eigen(cov(noisyPlane))$vectors
# TODO: Why this is needed only on my computer?
transform_mat2[,1] <- -1 * transform_mat2[,1]
transform_mat2[,2] <- -1 * transform_mat2[,2]

# Transform the original data by hand (multiply your data with the transformation matrix)
my_x <- noisyPlane %*% transform_mat2
