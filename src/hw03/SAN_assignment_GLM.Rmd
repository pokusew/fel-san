...
There seems to be a relationship between program and num_awards in our data. Academic program seems to have higher number of wards received. Although the feature prog2 (1 for Academic program, 0 otherwise) is highly correlated with math score.

Next, there seems to be a quadratic relationship  between work and num_awards in our data.

There we fit a new model using `poiss_v3_glm <- glm(num_awards ~ math + prog2 + poly(work, 2), family = "poisson", data = d3)` where a transformed feature prog2 has value 1 for Academic program, 0 otherwise.

Finally, we compare 5 different GLMs. We see that our **v3** model ("num_awards ~ math + prog2 + poly(work, 2)") is the best in terms of AIC (362.8) and BIC (382.6).
