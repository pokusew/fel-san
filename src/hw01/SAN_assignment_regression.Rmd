...
M1 <- matrix(
	...
	sample(coefs, replace = TRUE, size = n.dimensions^2),
	nrow = n.dimensions,
	ncol = n.dimensions,
)
...
M2.selection <- sample(n.dimensions, replace = FALSE, size = 80)
M2.unimportant <- M2.selection
# M2.unimportant <- M2.selection[26:50]

# M2[,M2.important] <- M2[,M2.important] * 10
# M2[,M2.unimportant] <- M2[,M2.unimportant] * 0.001
M2[,M2.unimportant] <- M2[,M2.unimportant] * (1/(coefs[M2.unimportant]*1000))

# KEEP THE CODE BELOW
noise <- rnorm(n.samples, sd = 8, mean = 0)
X2 <- X %*% M2
Y2_wo_noise <- (X2 %*% coefs)
Y2 <- (X2 %*% coefs) + noise
```
