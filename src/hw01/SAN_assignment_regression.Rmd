...
summary(learnAndTest(X, Y, "lm"))
```
**Your comment:** We violated noise normality by replacing the normal distribution with uniform distribution. Because the model had a relatively large dimension compared to the number of samples, the model started to overfit (nearly-perfect $R^2$, very low RSE) when we replaced the normal-distributed noise with uniform-distributed one.
...
M1.scale_factors <- rlnorm(n.dimensions, meanlog = 1)
M1.biggest <- sort(M1.scale_factors, index.return = TRUE, decreasing = TRUE)$ix[1:80]
# M1.fraction <- 0.2
# M1.selection <- sample(n.dimensions, replace = FALSE, size = round(M1.fraction * n.dimensions))
# M1.idxs <- M1.selection
M1.idxs <- M1.biggest
# M1[M1.idxs, ] <- sample(c(0, 1, 2, 3), replace = TRUE, size = length(M1[, M1.idxs]))
M1[M1.idxs, ] <- rlnorm(length(M1[M1.idxs, ]), meanlog = 1)
M1[, M1.idxs] <- 0
M1[col(M1) == row(M1)] <- M1.scale_factors
...
# my additional test code
X1.cov <- cov(X1)
X1.cor <- round(cor(X1), 2) > 0.9
...
```
**Justification:** Ridge works better than OLS when there is multicollinearity in the features.
