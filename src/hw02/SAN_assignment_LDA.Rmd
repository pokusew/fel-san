...
where $\overline{x}$ is a vector represents the overall mean of the data, $\mu_c$ represents the mean corresponding to each class, and $N_C$ are sizes of the respective classes.
...
**LDA was able to separate the given data nearly perfectly**. When trained on the whole dataset, only two 2 data points out of 178 were misclassified (i.e., ACC = 0.988764). Cross-validation on 10 random folds also confirmed this nearly-perfect performance of LDA on this dataset (mean ACC = 0.9888889).

Compared to the LDA, **PCA (without scaling)** performed very **bad** which was clearly visible from the plots. The reason for this is that there are big differences in the magnitude of variance between some variables (specifically variable 13 has a variance of several orders of magnitude larger than the rest). And because PCA aims to find directions which maximalize the variance in a data, this variable significantly affects the result.
However, if we pass `.scale = TRUE` to `prcomp`, the variables will be scaled to have unit variance before the (PCA) analysis takes place. When we performed **PCA with scaling**, we got much better results _(compared to the PCA without scaling)_, but there were still **more misclassifications** than in the LDA.

```r
# see that variable 13 is significantly affecting PCA without scaling
# compute the covarinace matrix
mydata.cov <- cov(mydata)
# see the variances of all variables (i.e., diagonal of the covarinace matrix)
print(mydata.cov[col(mydata.cov) == row(mydata.cov)])
# see the covarinace of variable 13
