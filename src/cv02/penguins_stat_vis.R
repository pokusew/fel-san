...
adelie.var <- sum((___ - ___)^2) / (___ - 1)
...
penguins_clean %>% 
  ...
  geom_density(aes(y=..density..)) + xlim(2000, 5000) +
  theme_minimal()
...
for(i in 1:nrow(summary_table)) {
  ...
  peng_plot <- penguins_clean %>% 
    filter(species == peng) %>% 
    ggplot(aes(x=body_mass_g, fill=species)) +
    geom_density(aes(y=..density..)) + xlim(2000, 7000) +
    # Draw mean and SD
