...
# by default poly tries to crate uncorrelated features
# that can be disabled using poly(raw = T)
...
# Extract the independent variables (the X matrix). Lasso will choose from them
BostonX <- Boston %>% dplyr::select(-medv) %>% as.matrix()

lasso_model <- glmnet(x = BostonX,
                      y = Boston$medv,
                      alpha = 1,
                      lambda = lambda_grid,
                      standardize = TRUE)

# See regularization in action. Think of what is this graph showing
plot(lasso_model, "lambda", label = TRUE)

# CV measures the performance of models associated with each lambda value
# Recall, that lambda controls the RMSE-vs-shrinkage trade-off.
lasso_cv_out <- cv.glmnet(x = BostonX,
                          y = Boston$medv,
                          alpha = 1)
# Plot lambda vs. RMSE
plot(lasso_cv_out)
...
# 2. Apart from lasso_cv_out$lambda.min, there is another option for the lambda parameter - lasso_cv_out$lambda.1se
#      Compare their values and find them in the plot(lasso_cv_out). What are the implications for the model
#      in terms of accuracy and complexity if the other is used?
